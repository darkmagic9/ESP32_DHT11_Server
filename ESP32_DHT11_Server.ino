...
static char celsiusLCD[7] = "00.00";
static char humidityLCD[7] = "00.00";
...
void setup() {
  pinMode(LEDR, OUTPUT);     // Initialize the LED_BUILTIN pin as an output
  ...
  xTaskCreatePinnedToCore(RecorddatabaseTask, "recorddatabase_task", 10000, NULL, 2, NULL, 0);
//  xTaskCreatePinnedToCore(SwitchTask, "switch_taskver_task", 10000, NULL, 5, NULL, 0);
//  xTaskCreatePinnedToCore(BlinkTask, "blink_task", 10000, NULL, 5, NULL, 0);
  xTaskCreatePinnedToCore(dhtTask, "dhtTask", 10000, NULL, 1, NULL, 1);
}
...
void BlinkTask(void *pvParameter) {
    ...
    while(true) {                // Wait for a second
      digitalWrite(LEDR, LOW);  // Turn the LED off by making the voltage HIGH
